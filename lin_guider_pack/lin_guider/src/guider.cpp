...
void guider::onStartStopButtonClick()
{
	...
	if( !is_started )
	{
		...
		if( save_drift )
		{
			res = fio::check_file_name( ui.lineEdit_DriftFileName->text().toLatin1().data() );
			switch( res )
			{
			case FIO_EXIST:
				u_msg( "File '%s' exists", ui.lineEdit_DriftFileName->text().toLatin1().data() );
				return;
			case FIO_ERROR:
				u_msg( "Unable to write file '%s'. Check permissions", ui.lineEdit_DriftFileName->text().toLatin1().data() );
				return;
			}

			m_logger->start( ui.lineEdit_DriftFileName->text().toLatin1().data() );
		}
